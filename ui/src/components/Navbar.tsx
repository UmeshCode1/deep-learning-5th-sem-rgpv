      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative w-14 h-14 bg-gradient-to-br from-purple-500 via-pink-500 to-blue-500 rounded-xl flex items-center justify-center shadow-3d-purple transform hover:rotate-12 hover:scale-110 transition-all duration-300 animate-pulse-glow group overflow-hidden">
            ...
            <div className="hidden md:block">
              <div className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-blue-400 animate-gradient">
                RGPV Bhopal
              </div>
              <div className="text-xs text-gray-400">Deep Learning — AL 503(B)</div>
            </div>
          </div>
...
function NavLink({href, icon, label}: {href: string, icon: string, label: string}){
  return (
    <a 
      href={href} 
      className="relative flex items-center gap-2 px-5 py-2.5 rounded-xl glass-dark border border-purple-500/20 hover:border-purple-500/40 text-gray-300 hover:text-white transition-all duration-300 transform hover:scale-105 hover:-translate-y-1 hover-lift group overflow-hidden"
    >
...
function MobileNavLink({href, icon, label, onClick}: {href: string, icon: string, label: string, onClick: () => void}){
  return (
    <a 
      href={href}
      onClick={onClick}
      className="flex items-center gap-3 px-4 py-3 rounded-lg glass-dark border border-purple-500/20 hover:border-purple-500/40 text-gray-300 hover:text-white transition-all duration-300 hover-lift group"
    >
